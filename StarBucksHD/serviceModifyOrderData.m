...
@implementation serviceModifyOrderData
@synthesize m_delegate = _m_delegate;

-(id)initWithDelegate:(id<serviceModifyOrderDataCallBackDelegate>)aDelegate 
...
- (void)passDataOutWithError:(Error*)error
{
	if (nil != self.m_delegate
		&& [self.m_delegate respondsToSelector:@selector(serviceModifyOrderDataCallBack: withErrorMessage:)])
	{
		[self.m_delegate serviceModifyOrderDataCallBack:m_dicReceiveData withErrorMessage:error];
	}
}
